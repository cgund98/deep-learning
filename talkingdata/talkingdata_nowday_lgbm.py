...
output_file = 'submits/lgbm_nowday_submit.csv'
...
#train_df = pd.read_csv(path + 'train.csv', skiprows=range(1,104903891), nrows=80000000, dtype=dtypes, usecols=train_cols)
...
#test_df = pd.read_csv(path + "test_supplement.csv", dtype=dtypes, usecols=test_cols)
...
def do_next_prev_Click( df,agg_suffix, agg_type='float32'):
    ...
    #### New added
    df['minute'] = df.click_time.dt.minute.astype('int8')
    df['second'] = df.click_time.dt.second.astype('int8')
...
def preproc_data(df):
    ...
    add_counts(df, ['ip',  'hour']) # <-- in antip's kernel
    ...
    df.drop(['ip', 'day', 'click_time', 'is_attributed', ], axis=1, inplace=True )
    gc.collect()

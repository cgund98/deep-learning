...
output_file = 'submits/lgbm_submit.csv'
#output_file = 'submits/lgbm_50m_submit.csv'
...
#train_df = pd.read_csv(path + 'train.csv', skiprows=range(1,84903891), nrows=50000000, dtype=dtypes, usecols=train_cols)
...
#test_supp_df = pd.read_csv(path + "test_supplement.csv", usecols=test_cols)
#test_supp_df = test_supp_df.dropna()
#test_supp_df['is_attributed'] = np.zeros(len(test_supp_df))
#test_supp_df = test_supp_df.astype(dtypes)
...
def preproc_data(df):
    ...
    #df['second'] = df['click_time'].dt.second.astype('uint8')  #> these drop lb by .0005, overfits
    ...
    df = do_countuniq( df, ['ip'], 'channel', 'X0', 'uint8', show_max=True ); gc.collect()
    df = do_countuniq( df, ['ip'], 'app', 'X3', 'uint8', show_max=True ); gc.collect()
    df = do_countuniq( df, ['ip'], 'device', 'X5', 'uint16', show_max=True ); gc.collect()
...
#test_len = len(train_df)
#train_df = pd.concat([train_df[common_cols], test_supp_df[common_cols]])

train_df = preproc_data(train_df)

#test_supp_df = train_df.iloc[test_len:]
#train_df = train_df.iloc[:test_len]
...
for i in range(0, 5):
    print('Training Model #{}'.format(str(i)), '\n')
    output_file = "lgbm{}_sub.csv".format(str(i))
    evals_results = {}
    ...
    #test_df['click_id'] = submit['click_id']
    #test_supp_df['is_attributed'] = model.predict(test_supp_df[inputs])
    ...
    #print('Supp:', test_supp_df.head())
    #print('Submit', test_df.head())

    submit['is_attributed'] = model.predict(test_df[inputs])

    print('Creating:', output_file)
    #test_supp_df.to_csv('submits/lgbm_supp.csv', index=False, float_format='%.9f')
